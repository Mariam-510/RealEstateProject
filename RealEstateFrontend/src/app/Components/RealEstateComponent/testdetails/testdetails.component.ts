...
export class TestdetailsComponent implements AfterViewInit,OnInit {
 ...
 ngOnInit(): void {
  ...
  onWindowScroll() {
    ...
    // Handle tab/nav bar stickiness
    if (this.tabLinks && this.tabLinks.nativeElement) {
      const tabBar = this.tabLinks.nativeElement;
      const tabBarOffset = tabBar.getBoundingClientRect().top + window.scrollY;
      
      if (scrollY >= tabBarOffset - 50) {
        tabBar.classList.add('sticky');
        console.log('sticky addded');
      } else {
    ...
    if (this.buyNowCard && this.buyNowCard.nativeElement) {
      ...
      const stopPoint = this.stopSection ? 
                        this.stopSection.offsetTop - card.offsetHeight - 500 : 
                        Number.MAX_SAFE_INTEGER;
      
      // Apply fixed position when scrolled past initial position but before stop point
      if (scrollY >= cardInitialOffset+300 && scrollY < stopPoint) {
      ...
      else if (scrollY < cardInitialOffset+300) {
        this.renderer.removeClass(card, 'fixed-event-card');
        console.log('fixed-event-card removed');
